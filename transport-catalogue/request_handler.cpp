...
namespace tr_cat::interface {
    ...
    void RequestInterface::AddStops() {
        std::for_each(stops_.begin(), stops_.end(),
                      [&](StopInput &stop) { catalog_.AddStop(stop.name, stop.coordinates); });
    }
    ...
    void RequestInterface::AddBuses() {
        std::for_each(buses_.begin(), buses_.end(),
                      [&](BusInput &bus) { catalog_.AddBus(bus.name, bus.stops, bus.is_ring); });
    }
