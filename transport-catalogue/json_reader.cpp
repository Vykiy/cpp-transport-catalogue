...
//-----------------------Parse Base------------------------------------
    void JsonReader::ParseBase(json::Node &base_node) {
        ...
        for (auto &element_node: base) {
            ...
            auto &type = element.at("type"s).AsString();
            if (type == "Stop"s) {
...
//-----------------------------Parse Render Settings--------------------------------------
    void JsonReader::ParseRenderSettings(json::Node &settings_node) {
        ...
        auto get_color = [](json::Node const &key, svg::Color &field) {
            if (key.IsString()) {
                field = key.AsString();
        ...
        if (settings.contains("color_palette"s)) {
            ...
            for (auto &node: array) {
                render_settings_.color_palette.emplace_back();
...
//--------------------------------------Print--------------------------------------
    void JsonReader::PrepareToPrint() {
        ...
        for (auto &answer: answers_) {
            to_document.push_back(visit(CreateNode{render_settings_}, answer));
        }
        document_answers_ = to_document;
    }
...
//-----------------------------CreateNode-----------------------------------
    ...
    json::Node JsonReader::CreateNode::operator()(int value) {
        return json::Dict{{"request_id"s,    value},
    ...
    json::Node JsonReader::CreateNode::operator()(BusOutput &value) {
        return json::Dict{{"request_id"s,        value.id},
    ...
    bool NodeCompare(const json::Node &lhs, const json::Node &rhs) {
...
} // namespace tr_cat::interface//tr_cat